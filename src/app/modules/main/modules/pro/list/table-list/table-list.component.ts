...
import { RuleService } from "../../../../snippets/services/rule.service";
...
export class ProTableListComponent implements OnInit {
  ...
  remove() {
    this.http
      .delete({ nos: this.selectedRows.map(i => i.no).join(",") })
      .subscribe(() => {
        this.getData();
        this.st.clearCheck();
      });
  ...
  add(tpl: TemplateRef<{}>) {
    this.modalSrv.create({
      ...
      nzOnOk: () => {
        this.loading = true;
        this.http.save({ description: this.description }).subscribe(() => {
          this.getData();
        });
      }
    });
